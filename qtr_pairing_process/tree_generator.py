...
class TreeGenerator:
    ...
    def generate_combinations(self, fNames, oNames, fRatings, oRatings):
        ...
        for name in fNames_sorted:
            ...
            # Outgoing pass: friendly team starts, so is_friendly_turn=True
            self.generate_nested_combinations(name, fnames_filtered, oNames_sorted, fRatings, oRatings, tree_top, is_friendly_turn=True)
            fNames_sorted[:] = uf.cycle_list(fNames_sorted)
        
    def generate_nested_combinations(self, first_fName, fNames, oNames, fRatings, oRatings, parent, is_friendly_turn):
        # Outgoing pass: is_friendly_turn=True (sort best for us), Return pass: is_friendly_turn=False (sort worst for us)
        ...
        # For each combination, get the best/worst rating for this friendly player
        def get_comb_rating(comb):
            rating_0 = fRatings[first_fName].get(comb[0], 0)
            rating_1 = fRatings[first_fName].get(comb[1], 0)
            return max(rating_0, rating_1) if is_friendly_turn else min(rating_0, rating_1)

        # Sort combinations according to whose turn it is
        combs_sorted = sorted(
            combs,
            key=get_comb_rating,
            reverse=is_friendly_turn
        )

        for comb in combs_sorted:
            rating_0 = fRatings[first_fName].get(comb[0], 'N/A')
            rating_1 = fRatings[first_fName].get(comb[1], 'N/A')
            # For display, always show both ratings
            display_rating = max(rating_0, rating_1) if is_friendly_turn else min(rating_0, rating_1)
            ...
            if fNames:
                ...
                for opponent, next_fName in opponent_perms:
                    ...
                    child_rating = fRatings[first_fName].get(opponent, 'N/A')
                    ...
                    # Alternate turn: next recursion is enemy's turn
                    self.generate_nested_combinations(
                        next_fName, nested_oNames, fNames, oRatings, fRatings, child_id, not is_friendly_turn
                    )
