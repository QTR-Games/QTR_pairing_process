...
class TreeGenerator:
    ...
    def generate_combinations(self, fNames, oNames, fRatings, oRatings):
        ...
        for name in fNames_sorted:
            ...
            self.generate_nested_combinations(fNames_sorted, oNames_sorted, fRatings, oRatings, tree_top)
            fNames_sorted[:] = uf.cycle_list(fNames_sorted)
            # oNames_sorted[:] = cycle_list(oNames_sorted)
            
        
    def generate_nested_combinations(self, fNames, oNames, fRatings, oRatings, parent):
        if not fNames:
            # print("NOT FNAMES")
            return
        ...
        for comb in combs_sorted:
            ...
            item_id = self.treeview.insert(parent, 'end', text=f"{first_fName} vs {comb[0]} ({rating_0}/5) OR {comb[1]} ({rating_1}/5)", values=remaining_fNames, tags=max(rating_0, rating_1))
            ...
            if remaining_fNames:
                for opponent in comb:
                    ...
                    self.generate_nested_combinations(nested_oNames, remaining_fNames, oRatings, fRatings, child_id)
