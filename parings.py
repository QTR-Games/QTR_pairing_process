...
# Function to get opponent player names from the grid
...
# Function to get friendly player names from the grid
...
def generate_combinations(fNames, oNames, textbox_bottom):
    def generate_nested_combinations(fNames, oNames, depth=0):
        if not fNames:
            return ""
    ...
    # Clear the textbox and insert new combinations
    textbox_bottom.config(state=tk.NORMAL)
    textbox_bottom.delete(1.0, tk.END)  # Clear the textbox
    
    # Get the friendly and opponent player names
    fNames_sorted = sorted(fNames, key=lambda x: x)
    oNames_sorted = sorted(oNames, key=lambda x: x)
    
    # Generate the nested combinations and insert into the textbox
    result = generate_nested_combinations(fNames_sorted, oNames_sorted)
    textbox_bottom.insert(tk.END, result)
    textbox_bottom.config(state=tk.NORMAL)
...
def create_ui():
    ...
    # Add a "Lists" button
    tk.Button(top_frame, text="Lists", command=lambda: select_directory_and_update_combobox(combobox1)).pack(side=tk.LEFT, padx=5)
    ...
    textbox_bottom = tk.Text(bottom_frame, height=6, wrap=tk.NONE)
    textbox_bottom.pack(side=tk.LEFT, padx=5)
    ...
    # Replace the original function call with the updated function
    tk.Button(bottom_frame, text="Generate", command=lambda: generate_combinations(get_friendly_player_names(), get_opponent_player_names(), textbox_bottom)).pack(side=tk.LEFT, padx=5)
    # tk.Button(bottom_frame, text="Generate", command=lambda: generate_combinations(get_friendly_player_names(), get_opponent_player_names(), textbox_bottom)).pack(side=tk.LEFT, padx=5)
    # tk.Button(bottom_frame, text="Generate", command=lambda: update_grid(textbox, grid_entries)).pack(side=tk.LEFT, padx=5)
    tk.Button(bottom_frame, text="Clear Texbox", command=lambda: clear_textbox(textbox_bottom)).pack(side=tk.LEFT, padx=5)
